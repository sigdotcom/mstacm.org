...
class AdvertForm extends React.Component<
  IAdvertProps & FormComponentProps,
  IAdvertState
> {
      ...
      <div>
        ...
        {confirmation && (
          <div>
            ...
            <p>
              Only then should you click this button, it will send to all
              selected lists
            </p>
            <Button type="danger" onClick={this.finalSend}>
              Confirm Send
            </Button>
          </div>
        )}
      ...
      personalizations: [
        {
          ...
          dynamic_template_data: {
            ...
            datetime: moment(data.dateHosted).format(
  ...
  private finalSend = (e: React.MouseEvent) => {
    ...
    const targets: IEmailAddress[] = values.emailGroups.map((addr: string) => {
      return { email: addr };
    });
    const body: MailData = this.createMessage(targets, values);
